...
if($text){
  $get = json_decode(file_get_contents("http://api-abaquran.aba.vg/handler.php?soura=".$text."&readernameEngilsh=Al_husari"));
  bot('sendaudio',[
    'chat_id' => $chat_id,
    'audio' => $get->audio,
    "reply_to_message_id"=>$message_id,
...
if(in_array($save,$soundafter)){
  $get = file_get_contents("http://api-abaquran.aba.vg/handler.php?soura=".$text."&readernameEngilsh=Al_husari");
  if(isset($ob->error)){
    bot("sendMessage",[
      "chat_id"=>$chat_id,
      "text"=>$ob->error,
